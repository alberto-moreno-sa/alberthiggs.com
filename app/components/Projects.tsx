...
function FeaturedProject({
  ...
  const { isExpanded, contentRef, contentHeight, triggerProps } = useExpandable();
    ...
    >
      ...
      <div className="p-6 md:p-8">
        ...
        <div className="flex items-start justify-between mb-4">
          ...
          <div className="flex items-center gap-3">
            {project.githubUrl && (
              <a href={project.githubUrl} target="_blank" rel="noopener noreferrer" className="text-text-muted hover:text-accent transition-colors" aria-label={`View ${project.name} on GitHub`}>
            ...
            {project.liveUrl && (
              <a href={project.liveUrl} target="_blank" rel="noopener noreferrer" className="text-text-muted hover:text-accent transition-colors" aria-label={`View ${project.name} live`}>
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {project.technologies.map((tech) => (
                    <span key={tech} className="px-2 py-0.5 text-xs font-mono text-accent/70 bg-accent/5 border border-accent/10 rounded">
                      {tech}
                    </span>
                  ))}
                </div>
    ...
    >
      <div className="flex items-start justify-between mb-3">
        ...
        <div className="flex items-center gap-2">
          {project.githubUrl && (
            <a href={project.githubUrl} target="_blank" rel="noopener noreferrer" className="text-text-muted hover:text-accent transition-colors">
          ...
          {project.liveUrl && (
            <a href={project.liveUrl} target="_blank" rel="noopener noreferrer" className="text-text-muted hover:text-accent transition-colors">
      ...
      <div {...triggerProps} className="cursor-pointer select-none focus:outline-none focus-visible:ring-2 focus-visible:ring-accent rounded">
        ...
        <div ref={contentRef}>
          <div className="pt-3 mt-3 border-t border-border/30">
            <div className="flex flex-wrap gap-1.5">
              {project.technologies.map((tech) => (
                <span key={tech} className="px-2 py-0.5 text-[10px] font-mono text-text-muted bg-surface border border-border/30 rounded">
                  {tech}
                </span>
              ))}
            </div>
...
export default function Projects({ projects, githubUrl }: { projects: Project[]; githubUrl?: string }) {
          ...
          {other.length > 0 && (
            <>
              ...
              <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {other.map((project, i) => (
                  <SmallProject key={project.slug} project={project} index={i} />
                ))}
              </div>
            </>
          )}
