...
interface CityIndex {
  ...
  source: string
  crs: string
  flight: { ground: number; heading: number; stations: Array<[number, number]> }
}
...
const BUILDING_MAT = new MeshLambertMaterial({ vertexColors: true, side: DoubleSide })
const TERRAIN_MAT = new MeshLambertMaterial({ vertexColors: true, side: DoubleSide })

/** Which building a triangle index belongs to, via the per-tile pick table. */
function pickBuilding(entry: TileEntry, faceIndex: number): Building | undefined {
...
function Scene({ cache, index }: { cache: TileCache; index: CityIndex }) {
  ...
  useFrame(() => {
    ...
    cityStore.setHover(
      b ? { building: b, screen: { x: pointer.current.x, y: pointer.current.y } } : null,
    )
  })
      ...
      {tiles.map(e => {
        ...
        return (
          <group key={e.key} position={[ox, oy, 0]}>
            {e.terrain && <mesh geometry={e.terrain} material={TERRAIN_MAT} />}
            {e.buildings && (
              <mesh
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, overflow: 'hidden' }}>
      <Canvas
        dpr={[1, 2]}
        gl={{ antialias: true }}
        camera={{ fov: 55, near: 1, far: 20000, position: [0, -700, 320], up: [0, 0, 1] }}
      >
